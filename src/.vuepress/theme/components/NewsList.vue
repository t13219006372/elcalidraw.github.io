<template>
  <div id="article-list" class="vp-article-list">
    ...
    <!-- <p class="sub-title" :data-item-count="String(items.length)">共{{ items.length }}条碎碎念~ (｡♥‿♥｡)</p> -->
    <figure>
      <img class="news-top-img" src="https://api.vvhan.com/api/acgimg" alt="图" />
    </figure>
    <template v-if="currentArticles.length">
      <template v-for="({ info, path }, index) in currentArticles" :key="index">
...
<script setup lang="ts">
  ...
  import type { Article } from '@vuepress/plugin-blog'
  ...
  const currentArticles = computed(() =>
    props.items.slice((currentPage.value - 1) * articlePerPage.value, currentPage.value * articlePerPage.value)
  )
  ...
  onMounted(() => {
    ...
    if (SUPPORT_PAGEVIEW)
      if (SUPPORT_PAGEVIEW) {
        void import(/* webpackChunkName: "pageview" */ 'vuepress-plugin-comment2/pageview').then(
          ({ updatePageview }) => {
            updatePageview()
          }
        )
...
<style lang="scss" scoped>
  h3 {
    ...
    background: linear-gradient(120deg, var(--theme-color-light), var(--theme-color) 30%, #50e3eb 100%);
