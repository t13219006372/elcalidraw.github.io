...
import { computed, onMounted, defineComponent, h, ref, nextTick, createApp, watchEffect, watch, provide } from 'vue'
...
export default defineComponent({
  ...
  setup() {
    ...
    const bingData = computed(() => (bingDatasRef.value ? bingDatasRef.value[bingIndex.value] : {}))
    const lDisabled = computed(() => (bingIndex.value == 0 ? true : false))
    const rDisabled = computed(() =>
      bingDatasRef.value && bingIndex.value == bingDatasRef.value.length - 1 ? true : false
    )
    const leftClick = () => {
      if (lDisabled.value) {
        return
            ...
            [
              ...
              h(DropTransition, { appear: true, delay: 0.08 }, () =>
                frontmatter.value.heroText === null
                  ? null
                  : h('h1', { class: 'vp-blog-hero-title' }, frontmatter.value.heroText || pageData.value.title)
              ),
...
#hitokoto {
  ...
  .word {
    ...
    #hitokoto_text {
      ...
      font-family: PRshouxie, Georgia, 'Times New Roman', Times, serif, sans-serif;
      font-size: 2.8rem;
      font-weight: 550;
